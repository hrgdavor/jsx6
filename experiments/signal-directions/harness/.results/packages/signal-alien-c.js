...
// libs/signal-alien/src/observe.js
...
// libs/signal-alien/src/signal.js
var ValueSymbol = Symbol.for("signalValue");
var srcSymbol = Symbol.for("signalAlienSource");
var noOp = function() {
};
function signal2(value, name) {
...
function prepareSignal(value, name) {
  ...
  const $signal = (...args) => {
    if (args.length === 0)
      return getActiveSub() === void 0 ? current : src();
    if (setValue(args[0])) {
      fireChanged();
      return true;
    }
...
// libs/signal-alien/src/computed.js
var batch = (fn) => {
  startBatch();
  try {
    return fn();
...
function createComputedSignal(getValue, { eager = false, declaredDeps = [], name } = {}) {
  const listeners = /* @__PURE__ */ new Set();
  let node;
  let pushStop;
...
var $C = (getValue, ...declaredDeps) => createComputedSignal(getValue, { declaredDeps });
var $CE = (getValue, ...declaredDeps) => createComputedSignal(getValue, { eager: true, declaredDeps });
var dispose = ($computed) => $computed?.dispose?.();

// libs/signal-alien/src/state.js
var mergeValueSymbol = Symbol.for("signalMergeValue");
var stateSymbol = Symbol.for("signalState");
...
// libs/signal-alien/src/compat.js
...
function makeCompat(core) {
  ...
  const observeNow2 = (obj, callback) => observe3(obj, callback, true);
...
function createDerivedSignal(deps, getValue) {
  if (deps.some(isOpaqueDep))
    return createLegacyDerivedSignal(deps, getValue);
  return createComputedSignal(getValue, { eager: true, declaredDeps: deps });
}
function $S(template, ...signals) {
  if (!signals.length)
    return signal2(template);
...
// experiments/signal-directions/harness/.results/packages/signal-alien-c.entry.js
var $a = signal2(0);
var $b = signal2(1);
var $sum = $S(() => $a() + $b(), $a, $b);

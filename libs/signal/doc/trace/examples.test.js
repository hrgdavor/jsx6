/**
 * Runs every example in `doc/trace/examples/`.
 *
...
const EXAMPLES = ['watch.js', 'source-ref.js', 'state.js', 'recording.js', 'session.js']

for (const file of EXAMPLES) {
  test(`trace doc example: ${file}`, async () => {
    await import(`./examples/${file}`)
    expect(true).toBe(true)
  })
}

test('every trace example file is listed above', async () => {

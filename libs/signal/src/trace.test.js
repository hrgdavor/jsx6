...
test('options belong to a session: a later traceSignals() restores the defaults', () => {
  // The cheapest possible session still has an origin — an option set for one session must not silently
  // degrade the next one, which is exactly what "debugging stopped reporting origins" looks like.
  traceSignals({ origin: false })
  expect(traceSignals(true)).toBe(true) // starts a new session, defaults restored
  const $x = signal(1, 'x')
  expect(signalTrace($x).origin).not.toBeNull()
  traceSignals(false)
})

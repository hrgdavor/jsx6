 ...
 * ## Why this exists
 *
 * A signal is a function, and Chrome (and Node's inspector) render a function value as source text: they
 * list none of the function's own properties, so a `value` getter on a signal is invisible, and DevTools
 * does not run custom formatters for function values either. The only shapes DevTools will format are
 * proxies — that route was implemented, measured, and **rejected**: a proxy handle per signal costs ~13×
 * on the read path (2M write+read 26.75 ms → 349.93 ms, computed reads +2373 %), and no dev aid is worth
 * that. What is left is the console interception below, plus the `value` getter for direct use.
 *
 ...
 *
 * This deliberately does *not* reuse the frame filter in `src/trace.js`: that filter skips "the
 * library", and a capture made from inside this module cannot tell the library from the file it is
 * standing in without a self-reference, while the local name check below is both simpler and exact.
 * (Sharing it was tried; it reported the wrapper itself and broke `showCallSite`'s test.)
 */
const callerSite = () => {
  try {

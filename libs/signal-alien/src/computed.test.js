...
test('union dependencies: an omitted read is still tracked', () => {
  const $a = signal(1)
  const $b = signal(2)
  // The omission is the point: union dependencies track it anyway.
...
test('a declared dependency the callback never reads is still subscribed', () => {
  ...
  // which neither alien's graph nor a collector can see, so the declared list is the only way the
  // derived signal can learn that the value changed.
  let value = 1
...
test('a computed cycle is reported once and still returns the previous value', () => {
  // alien's computed breaks a re-entrant read by returning the previous value — no hang, no overflow,
  // and by itself no error. The report is what makes a cyclic graph findable.
...
test('dependency tracking is inert outside a computation', () => {
  // Unlike the native core there is no `track.js` collector to poke at here: dependencies are alien's
  // graph. What must hold is that a bare read outside any computation subscribes to nothing.
  const $a = signal(1)
  const before = $a()
  $a(2)
  expect($a()).toBe(2)

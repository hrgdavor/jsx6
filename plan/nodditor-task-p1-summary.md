...
- `bun run start` (dev server) can crash on transient files under `static/` (watcher copies a `.tmpdir` that vanished → ENOENT in tools/build copyTask). Re-run it; cosmetic, not editor code.
